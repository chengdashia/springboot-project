...
public class FileInfoServiceImpl extends ServiceImpl<FileInfoMapper, FileInfo> implements FileInfoService {
    ...
    public R<Object> getFileRandomTen() {
        try {
            int count = Math.toIntExact(fileInfoMapper.selectCount(new QueryWrapper<FileInfo>().select("1")));
            int random = 0;
            int max = 10;
            if(count > max){
                random = RandomUtil.randomInt(0, count - 10);
            }
            Page<Map<String, Object>> mapPage = fileInfoMapper.selectMapsPage(
                    (new Page<>(random, 10)), new QueryWrapper<FileInfo>()
